...
# Managed by launchd (com.taxiai.emulator2.plist, KeepAlive) — do not run twice:
...
(
  ...
  sleep 25
  "$ADB" -s emulator-5556 emu geo fix "$PARK_LNG" "$PARK_LAT"
) &
...
exec "$SDK/emulator/emulator" -avd radar2 -port 5556 \
